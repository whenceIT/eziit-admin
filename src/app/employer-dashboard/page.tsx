...
import { TotalEmployees } from "@/components/dashboard/employerOverview/employees";
import { TotalMerchants } from "@/components/dashboard/employerOverview/merchants";
import { Outstanding } from "@/components/dashboard/employerOverview/outstandingPayments";
import { Processed } from "@/components/dashboard/employerOverview/processedPayments";
...
async function fetchTransactions(user: User | null): Promise<Transaction[]> {
  ...
  const url =
    user.user_type === "employer"
      ? `https://ezitt.whencefinancesystem.com/transactions?limit=6&employerId=${user.id}`
...
export default function employerDashboard(): React.JSX.Element {
  ...
  return (
    <Grid container spacing={3}>
      <Grid lg={3} sm={6} xs={12}>
        <TotalEmployees diff={16} trend="down" sx={{ height: "100%" }} title="Number of Clients" />
      </Grid>
      <Grid lg={3} sm={6} xs={12}>
        <TotalMerchants diff={16} trend="down" sx={{ height: "100%" }} title="Number of Employers" />
      </Grid>
      <Grid lg={3} sm={6} xs={12}>
