...
export default function EmployerClientsPage(): React.JSX.Element {
    ...
    async (status: string | null) => {
      ...
      try {
        ...
        const filteredRequests = data.filter((req: any) => {
          const isValidType = req.request_type === "underwriter-client" || req.request_type === "client-underwriter"
  ...
  return (
    <>
      <Head>
        <title>{`Underwriter-Client Relationships | Dashboard | ${config.site.name}`}</title>
      </Head>

      <Stack spacing={3}>
        <Stack direction="row" spacing={3}>
          <Stack spacing={1} sx={{ flex: "1 1 auto" }}>
            <Typography variant="h5">Underwriter-Client Relationships</Typography>
